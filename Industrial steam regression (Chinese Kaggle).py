...
def load_train_data():
    df = pd.read_csv("/Users/mikexie/zhengqi_train.txt", header=0, sep="\s+")
    #print(df.describe())
...
def load_test_data():
    df = pd.read_csv("/Users/mikexie/zhengqi_test.txt", header=0, sep="\s+")
    #print(df.describe())
    X_test = df
    return X_test
...
#    lgbm = lightgbm.LGBMRegressor(learning_rate=0.01, n_estimators=500, num_leaves=31)
    ...
    return svr, line, lasso, ENet, KRR1, KRR2, xgb, nn #, lgbm
...
#    svr, line, lasso, ENet, KRR1, KRR2, lgbm, xgb, nn = build_model()
    svr, line, lasso, ENet, KRR1, KRR2, xgb, nn = build_model()
...
#    averaged_models = AveragingModels(models = (svr,KRR2,lgbm,nn))
    averaged_models = AveragingModels(models = (svr,KRR2,nn))
...
# Thanks for the idea of using stacking given by TianChi player. I modified that lightBGM is not used.
